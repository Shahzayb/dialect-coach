...
- When cutting a release: bump the version in `pyproject.toml`, tag the commit in Git, and
  push the tag — `.github/workflows/release.yml` creates the GitHub Release from it
  (`gh release create vX.Y.Z --generate-notes --verify-tag`), so do not create it by hand.
